...
public final class Expression {
    ...
    public static Node buildTree (final String expr) {
        final Node n = new Node ();
        String expr2 = expr.trim ();
        while ((expr2.charAt (0) == '(') && (expr2.charAt (expr2.length () - 1) == ')')) {
    ...
    private static double op (final char code, final double op1, final double op2) {
        double res = 0;
        switch (code) {
            case '+' :
                res = op1 + op2;
    ...
    private static double replaceEnv (final Map<String, Double> env, final String id) {
        double res = 0;
    ...
    public static double valueOfExpr (final String expr) {
        return Expression.valueOfTree (Expression.buildTree (expr), new HashMap<String, Double> ());
    }

    public static double valueOfExpr (final String expr, final Map<String, Double> env) {
    ...
    public static double valueOfTree (final Node n, final Map<String, Double> env) {
        final int moinsdeux = -2;
        if ((Expression.priority (n.getLetter ().charAt (0)) != moinsdeux) && (n.getLeft () != null) && (n.getLeft () != null)) {
            return Expression.op (n.getLetter ().charAt (0), Expression.valueOfTree (n.getLeft (), env), Expression.valueOfTree (n.getRight (), env));
        }
